...
export function Sidebar({
  ...
  return <div className={cn("fixed top-0 left-0 z-40 h-full transition-all duration-300 bg-white border-r border-border", expanded ? "w-64" : "w-20", className)}>
      <div className="flex items-center justify-between p-4 border-b border-border h-16">
        <div className={cn("flex items-center", expanded ? "justify-between w-full" : "justify-center")}>
          {expanded && <h1 className="text-xl font-bold text-gtalk-primary">gTalk Send</h1>}
      ...
      <div className="flex flex-col p-3 space-y-2 overflow-y-auto h-[calc(100vh-64px)]">
        <NavItem icon={<BarChart />} label="Dashboard" path="/" active={location.pathname === '/'} expanded={expanded} />
        <NavItem icon={<MessageSquare />} label="Messaging" path="/messaging" active={location.pathname === '/messaging'} expanded={expanded} />
        <NavItem icon={<Users />} label="Contacts" path="/contacts" active={location.pathname === '/contacts'} expanded={expanded} />
...
function NavItem({
  ...
  return <Link to={path} className={cn("flex items-center p-3 rounded-md transition-colors", active ? "bg-gtalk-primary bg-opacity-10 text-gtalk-primary" : "text-gray-600 hover:bg-gray-100", !expanded && "justify-center")}>
